...
def render_layout(*, title: str, active_nav: str, body: str, script: str = "") -> str:
    ...
    return (
        ...
        "      margin: 0 auto 8px;\n"
        "    }\n"
        "    .book-title { margin: 0 0 6px; font-size: 16px; line-height: 1.35; }\n"
        "    .book-meta { font-size: 12px; color: var(--muted); margin-bottom: 6px; }\n"
        "    .book-actions { display: flex; flex-wrap: wrap; gap: 6px; }\n"
    ...
    *,
    ...
    for book in page_books:
        ...
        cards.append(
            "<div class=\"book-card\">"
            f"{cover_html}"
            f"<h3 class=\"book-title\">{escape(book['title'])}</h3>"
            f"<div class=\"book-meta\">{provider_badge}</div>"
            f"<div class=\"book-meta\">已下载：{escape(book.get('last_downloaded_chapter_title') or '-')} / #{escape(format_chapter_number(book.get('last_downloaded_chapter_number')))}</div>"
            f"<div class=\"book-meta\">最新：{escape(book.get('latest_site_chapter_title') or '-')} / #{escape(format_chapter_number(book.get('latest_site_chapter_number')))}</div>"
            f"<div class=\"book-meta\">待更新：{pending} | 追更：{follow_text} | 检查：{escape(fmt_time(book.get('last_checked_at', '')))}</div>"
